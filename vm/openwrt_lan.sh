...
function advanced_settings() {
  ...
  if VLAN2=$(whiptail --backtitle "Proxmox VE Helper Scripts" --inputbox "Set a LAN Vlan (leave blank for default)" 8 58 --title "LAN VLAN" --cancel-button Exit-Script 3>&1 1>&2 2>&3); then
    if [ -z $VLAN2 ]; then
      VLAN2="Default"
      LAN_VLAN=""
    else
...
send_line_to_vm "uci add_list network.lan.dns=${LAN_GATEWAY}"
send_line_to_vm ""
send_line_to_vm "uci delete dhcp.wan"
send_line_to_vm "uci delete dhcp.lan"
send_line_to_vm "uci delete dhcp.@dnsmasq[0]"
send_line_to_vm ""
...
qm set $VMID \
  -net0 virtio,bridge=${LAN_BRG},macaddr=${LAN_MAC}${LAN_VLAN}${MTU} >/dev/null 2>/dev/null
